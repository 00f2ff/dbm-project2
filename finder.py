...
def save_restaurants(category_filter):
    # load CSV data
    cities, states, populations = load_location_data('population1.csv')
    # create dict
    cat = {category_filter: []}
    ...
    with open("{0}_A.json".format(category_filter),"w") as f:
        f.write(cat)
...
def analyze(category_filter):
    ...
    cities, states, populations = load_location_data('population1.csv')
    # load JSON data
    with open("{0}_A.json".format(category_filter),"r") as f:
        cat = f.read()
    cat = json.loads(cat)
    ...
    # save data to JSON
    cat = str(json.dumps(cat, indent=2))
    with open("{0}_B.json".format(category_filter),"w") as f:
        f.write(cat)
